...
3. Pipeline внутри `services/video_stream` работает как `StreamReader -> FrameQueue -> Preprocessor -> ProcessedFrameQueue -> Consumer`.
4. API открывает активный `VideoSegment` и пишет метаданные кадров в PostgreSQL и `storage/videos/.../frames.jsonl`.
5. `GET /cameras/{id}/snapshot` отдает живой обработанный кадр.
6. `POST /frames/{frame_id}/analysis` принимает детекции/события от будущего analyzer/worker.

Политика очередей:

* `FrameQueue` и `ProcessedFrameQueue` используют `Queue(maxsize=N)`.
* При переполнении применяется фиксированная политика `drop oldest`.
* Причина: pipeline не накапливает устаревшие кадры и остаётся ближе к реальному времени.

Явные ответственности:

* `StreamReader` только читает и нормализует кадры.
* `ScenePreprocessor` только считает признаки и формирует новый `ProcessedFrame`.
* `CameraManager` только оркестрирует worker-потоки и dispatch в consumers.
